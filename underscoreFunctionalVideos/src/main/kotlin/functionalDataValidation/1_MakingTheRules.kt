...
class Address(val number:Int, val street:String)
...
object Pass:Result()
class Fail(val message:List<String>):Result()

fun <T> rule(f:(T) -> Result) {}
typealias Rule<T> = (T) -> Result

val nonEmpty:Rule<String> = { value:String -> if (value.isEmpty()) Fail(listOf("Empty String")) else Pass }

fun gte(min:Int):Rule<Int> = { value:Int -> if (value < min) Fail(listOf("Too small")) else Pass }

// we need to build a combinator to get the fail or the pass of the two rules

val checkAddress:Rule<Address> = { address:Address -> gte(1)(address.number) and nonEmpty(address.street) }
